...
export default function TrainingPostTemplate(
  ...
  return (
    <div>
      <div
        className={`grid h-[872px] ${props.items.length >= 3 ? 'grid-cols-1 grid-rows-2 lg:grid-cols-2 lg:grid-rows-1' : ''} gap-8 px-6`}
      >
      ...
      <div
        className={`mt-8 grid h-[350px] gap-8 px-6 ${props.items.length == 4 ? 'grid-cols-1 lg:grid-cols-1' : 'grid-cols-1 lg:grid-cols-2'} ${props.items.length < 4 ? 'hidden' : 'flex'}`}
      >
      ...
      {
        <PostPublicItemView
          className='mt-8 h-[416px] w-full px-6'
          data={props.items[5]}
        />
      }
    </div>
