...
export default function AlbumComponent() {
    ...
    const [albums, setAlbum] = useState<AlbumPublicType[]>([]);
    ...
    return (
        ...
        >
            ...
            <div className='grid  h-screen items-center justify-items-center bg-[#F2F1ED] flex-col '>
                {albums.map((item, index) => {
                    return <AlbumGroup item={item} key={index} background={(index % 2 == 0) ? "bg-[#EFE5D2]" : "bg-[#DFDAD4]"} />
                })}
            </div>
        </PageContainer>
    )
